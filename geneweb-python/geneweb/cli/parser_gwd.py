...
class GenewebCLI:
    ...
    def _add_global_options(self):
        ...
        g.add_argument(
            "-version",
            action="store_true",
            help="Print the GeneWeb version and exit")
        g.add_argument(
            "-hd",
            help="Root directory containing etc/, images/, lang/",
            default="gw")
        g.add_argument("-etc_prefix", help="Directory for etc/", default=None)
        g.add_argument(
            "-bd",
            "--base-dir",
            help="Directory where databases are installed",
            default=BASE_DIR,
        )
        ...
        g.add_argument(
            "-blang",
            action="store_true",
            help="Select language according to browser")
        g.add_argument(
            "-p", "--port", type=int, default=2317, help="HTTP listening port"
        )
        g.add_argument(
            "-a", "--address", default="0.0.0.0", help="Listening IP address"
        )
    ...
    def _add_server_options(self):
        ...
        s.add_argument(
            "-cgi_secret_salt",
            help="Secret salt to secure CGI forms")
        s.add_argument(
            "-conn_tmout",
            type=int,
            default=120,
            help="Connection timeout (Unix)")
        s.add_argument(
            "-n_workers",
            type=int,
            default=20,
        ...
        s.add_argument(
            ...
            type=int,
            default=150,
            help="Maximum pending requests",
        )
        s.add_argument(
            "-cache_in_memory",
            help="Database to preload in memory")
        s.add_argument(
            "-cache_langs",
            nargs="*",
            help="Languages to preload in cache")
    ...
    def _add_security_options(self):
        ...
        sec.add_argument(
            "-digest",
            action="store_true",
            help="Enable HTTP Digest Auth")
        sec.add_argument("-friend", help="Friend account password")
        sec.add_argument("-wizard", help="Wizard account password")
        sec.add_argument(
            "-wjf",
            action="store_true",
            help="Downgrade wizard to friend")
        sec.add_argument(
            "-login_tmout",
            type=int,
            default=1800,
            help="Login session timeout (CGI)")
        sec.add_argument(
            "-trace_failed_passwd",
            action="store_true",
            help="Log failed passwords")
    ...
    def _add_misc_options(self):
        ...
        m.add_argument(
            "-predictable_mode",
            action="store_true",
            help="Disable randomness for testing",
        )
        ...
        m.add_argument(
            ...
            type=int,
            default=6,
            help="Minimum requests to log robot activity",
        )
        m.add_argument(
            "-max_clients",
            type=int,
            help="Maximum simultaneous clients (deprecated)")
