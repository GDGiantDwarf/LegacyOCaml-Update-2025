...
class CalendarRepository:
    ...
    def add_calendar_date(
        ...
        """Ajoute une date avec conversion automatique vers tous les calendriers"""
    ...
    def get_calendar_dates_by_person(self, person_id: int) -> List[CalendarDate]:
        """Récupère toutes les dates d'une personne"""
        return self.session.query(CalendarDate).filter(
            CalendarDate.person_id == person_id
        ).all()

    def get_calendar_date_by_id(self, date_id: int) -> Optional[CalendarDate]:
        """Récupère une date par son ID"""
        return self.session.query(CalendarDate).filter(
            CalendarDate.id == date_id
        ).first()
