...
class ChildRepository:
    ...
    def add_child(self, person_id: int, family_id: int, relation_type: str = None):
        if not person_id:
    ...
    def get_child_by_id(self, child_id):
        return self.session.query(ChildInFamily).filter(ChildInFamily.id == child_id).first()
    
    def update_child_by_id(self, 
        child_family_id: int, 
        person_id: int = None,
        family_id: int = None,
