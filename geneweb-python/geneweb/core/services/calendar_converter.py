...
class CalendarConverter:
    ...
    JULIAN_MONTHS = GREGORIAN_MONTHS
    ...
    def gregorian_to_julian(year: int, month: int, day: int) -> Dict[str, int]:
        jdn = CalendarConverter.gregorian_to_julian_day_number(
            year, month, day)
    ...
    @staticmethod
    def gregorian_to_french_republican(
        year: int, month: int, day: int
    ) -> Optional[Dict]:
        epoch = date(1792, 9, 22)
        end_date = date(1805, 12, 31)
        ...
        return {
            ...
            "month": french_month,
            "month_name": (
                CalendarConverter.FRENCH_MONTHS[french_month - 1]
                if french_month <= 12
                else "Sansculottides"
            ),
            "day": french_day,
    ...
    def convert_from_gregorian(year: int, month: int, day: int) -> Dict:
        ...
        french = CalendarConverter.gregorian_to_french_republican(
            year, month, day)
        ...
        return {
            ...
            "julian": {
                **julian,
                "month_name": (
                    CalendarConverter.JULIAN_MONTHS[julian["month"] - 1],
                )
            },
